...
for nowtimes in range(2, int(times)+1): #用 range 開始的數字來調整從哪個測站開始
    ...
    for filename in filelist:
        if(number in filename):
            shutil.move('C:\\Users\\GIGABYTE\\Downloads\\' + filename, "C:\\Users\\GIGABYTE\\桌面\\VScode_Code\\天氣預測資料\\自動雨量站\\" + '{}({})\\'.format(name, number) + filename)
