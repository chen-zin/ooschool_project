...
driver.find_element(By.CSS_SELECTOR, '#agr').click()
...
for nowtimes in range(1, int(times)+1): #用 range 開始的數字來調整從哪個測站開始
    ...
    for filename in filelist:
        if(number in filename):
            shutil.move('C:\\Users\\GIGABYTE\\Downloads\\' + filename, "C:\\Users\\GIGABYTE\\桌面\\VScode_Code\\天氣預測資料\\農業站\\" + '{}({})\\'.format(name, number) + filename)
